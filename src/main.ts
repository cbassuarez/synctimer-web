...
import { generateSvgMarkup, downloadSvg, downloadPng } from './qr';
import {
  GeneratorConfig,
  HostEntry,
  Mode,
...
import { parseAnyInput, parseHostShareLinks, parseJoinLink } from './parse';
import { copyText } from './ui';
import { decodeState, encodeState, loadPersistedConfig, persistConfig } from './storage';
import '../styles.css';
      ...
      <main className="layout">
        <section className="left">
          ...
          <Step title="Step 2: Hosts">
            <div className="field">
              <label>Host Share Link(s) or join link</label>
              <textarea
                rows={3}
                value={hostInput}
              ...
              <AnimatePresence>
                {showManual && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                    ...
                    <button onClick={handleManualAdd} className="secondary">Add host</button>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
